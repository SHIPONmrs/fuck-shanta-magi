...
def banner():
	clear()
	alvino_xy(f'''\t
     {m} _   _   ___   _________________ 
       {k}| \ | | / _ \ |___  /_   _| ___ \
       {h}|  \| |/ /_\ \   / /  | | | |_/ /
       {b}| . ` ||  _  |  / /   | | |    / 
       {u}| |\  || | | |./ /____| |_| |\ \ 
     {p}\_| \_/\_| |_/\_____/\___/\_| \_|
    
     
	                 {m}★{k}★{h}★SHIPON{u}{h}★{k}★{m}★{u}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t             LOVE SHOWING: [green]HAPPY FOREVER[purple] '))
		...
		print(f'  %s[%sx%s]%s LOGIN FAIL.....CEK TUMBAL LUU NGAB !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang [green]%s[purple] ILLEGITIMATE CHILD'%(my_name)))
	alvino_xy(f'{u}ID  : '+str(my_id))
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold cyan]           • CHOOSE WHY YOU WANT • [/bold cyan]'))
	print('')
...
def lah():
	print("\r"+balmond+m+" \x1b[1;95mTotal ID Which Collected :\x1b[1;97m "+str(len(id))+"                     ")
...
def grup1(urls):
	...
	print(balmond+m+" \x1b[1;94mSedang Collecting ID")
	print(balmond+m+" \x1b[1;94m Press CTRL + C For Stop")
	while True:
		try:
			ua = 'Mozilla/5.0 (SymbianOS/9.3; Series60/3.2 NokiaE52-1/052.003; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 BrowserNG/7.2.6.2 3gpp-gba'
			miskinlu = {"user-agent": ua}
...
#		taplikasi.append('no')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]•[yellow]] Masukkan Additional Password Minimum 6 Characters\n[[purple]•[yellow]] Contoh :[green] kakak,ngentod,adik[purple] '))
